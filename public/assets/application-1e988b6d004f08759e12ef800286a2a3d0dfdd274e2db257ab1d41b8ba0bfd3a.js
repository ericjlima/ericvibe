...
$(function(){
  ...
  //$(document).on('click','#awebdeveloper > h1:nth-child(1)',function(){
  // $('#awebdeveloper > h1:nth-child(1)').addClass('eric_fadein');
  //});

  var transEffect = function(){
     if($(window).scrollTop() > 0){
       var fromTop = ($(window).scrollTop() + ($(window).height()/1.5));
       var target = $('.eric__transeffec').eq(i);
       if($(target).offset().top < fromTop){
          $(target).addClass('eric_fadein');
...
// ! ---------
	...
	$(window).scroll(function(e){
                ...
                var perc = ((window.innerHeight)*0.01)*50;
